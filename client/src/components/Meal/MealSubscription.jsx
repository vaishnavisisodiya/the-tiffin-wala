...
function MealSubscription() {
  ...
  // Determine the minimum time based on the selected date
  const getMinTime = () => {
      ...
      <form action="" className='flex flex-col gap-2' onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <div className='flex items-center border bg-white w-full'>
            ...
            <input type="date" value={date} name="phone" placeholder='Select Date to deliver' className='w-full h-full px-2 py-2 border-l focus:outline-none' id="date" required onChange={(e) => setDate(e.target.value)} min={formattedDate} />
          </div>
        </div>
        <div>
          ...
          <div className='flex items-center border bg-white w-full'>
            ...
            <input type="time" value={time} name="phone" placeholder='Select Time to deliver' className='w-full h-full px-2 py-2 border-l focus:outline-none' id="time" required onChange={(e) => setTime(e.target.value)} min={getMinTime()} />
          </div>
        </div>
        <div className=''>
